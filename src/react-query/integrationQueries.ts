 ...
 *   - `mutateAsync`: A function to call the mutation.
 */
function useCreateDriftAssistSecret() {
  const { createDriftAssistSecret } = useIntegrationService();

  const { mutateAsync } = useMutation({
    mutationFn: (obj: DriftAssistSignInRequest) => createDriftAssistSecret(obj),
  });
 ...
 * @returns An object with the following properties:
 *   - `refetch`: A function to refetch the data.
 *   - `data`: The list of Drift Assist secrets.
 *   - `isLoading`: A boolean indicating whether the data is being fetched.
 *   - `isError`: A boolean indicating whether the data fetch failed.
 */
function useGetDriftAssistIntegrationsByProjectId(project_id: string) {
  const { getDriftAssistIntegrationsByProjectId } = useIntegrationService();

  const { refetch, data, isLoading, isError } = useQuery({
    queryKey: [QUERY_KEY.GET_DRIFT_ASSIST_INTEGRATIONS_BY_PROJECTID, project_id],
    queryFn: () => getDriftAssistIntegrationsByProjectId(project_id),
    enabled: !!project_id,
    refetchOnWindowFocus: false,
  });
 ...
 *   - `mutateAsync`: A function to call the mutation.
 */
function useGetDriftAssistSecret() {
  const { getDriftAssistSecret } = useIntegrationService();

  const { mutateAsync, isPending } = useMutation({
    mutationFn: (integration_id: string) => getDriftAssistSecret(integration_id),
  });
